...
namespace _291_Group2
{
    public partial class Form1 : Form
    {
        ...
        {
            string query = "SELECT ReturnDate from Rentals WHERE ";
            switch (mode)
            {
                case "Customer":
                    query += $"CID = {CID.Text}";
                ...
                case "Branch":
                    query += $"PickupBID = {BID.Text} OR ReturnBID = {BID.Text}"; 
                    break;
            }
            if (con.State != ConnectionState.Open)
            {
        ...
        {//Add Car Button
            ...
            else if (dt.Rows.Count == 0)
            {
                ...
                myCommand.CommandText = $"insert into Car (VIN, Make, Model, Year, NumberofSeats, Color, InsuranceNumber, OdometerNumber, BID, CarTypeID) values " +
                    $"( '{VIN.Text}', '{Make.Text}', '{Model.Text}', '{Year.Text}', '{Seats.Text}', '{Color.Text}', '{CInsurance.Text}', '{Odometer.Text}', '{CBID.Text}', '{CCarType.Text}')";
            ...
            else if (VIN.Text != "")
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    if (warning_result == DialogResult.Yes)
                    {
                        ...
                        {
                            MessageBox.Show("Can not delete Car. Pending Rental Transaction is found.");
                        }
        ...
        private void Tabs_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (Customer.SelectedTab == Customer.TabPages["rentals_tab"])
            {
                    ...
                    SqlDataAdapter pickup_branch_adapter = new SqlDataAdapter("Select BID, (Street_address1 + ', ' + City) AS Location FROM Branch", con);
                    DataTable dt = new DataTable();
                ...
                //----rentals dropoff branch box--------------
                    SqlDataAdapter dropoff_branch_adapter = new SqlDataAdapter("Select BID, (Street_address1 + ', ' + City) AS Location FROM Branch", con);
            ...
            else if (Customer.SelectedTab == Customer.TabPages["car_tab"])
            {
                ...
                CCarType.DisplayMember = "CarTypeID";
        ...
        private void actual_return_date_ValueChanged(object sender, EventArgs e)
        {
            show_rate();
        }
        ...
        private void comboBox3_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            ...
            //string q = "Display customers name over between 25 and 30 years old, who rented a 'non white', 'Full - size SUV' car outside Edmonton,  and returned it in Edmonton. ";
